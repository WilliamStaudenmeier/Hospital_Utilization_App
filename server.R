function(input, output, session) {
  ...
  output$vbox <- renderInfoBox({
    infoBox(
      "Progress", paste0(input$mean, "%"), icon = icon("list"),
      color = "purple"
    )
  })
  ...
  output$plot1 <- renderPlotly({
    ...
    capacityPatientRoom1 = as.numeric(input$room1  )
    
    capacityPatientRoom2 = as.numeric(input$room2  )
    
    capacityPatientRoom3 = as.numeric(input$room3  )
    
    capacityPatientRoom4 = as.numeric(input$room4  )
    
    capacityPatientRoom5 = as.numeric(input$room5  )
    ...
    data$PatientRoom1 = round(pmin((1/3) * data$Emergency, capacityPatientRoom1),0)
    
    
    data$PatientRoom2 = round(pmin((1/3) * data$Emergency, capacityPatientRoom2),0)
    
    data$PatientRoom3 = round(pmin((1/3) * data$Emergency, capacityPatientRoom3),0)
    ...
    data$PatientRoom5 = round(pmax(pmin(data$Emergency - data$PatientRoom1 - data$PatientRoom2 - data$PatientRoom3-data$waitingPatientRoom1 -
                                          data$PatientRoom4, capacityPatientRoom5),0),0)
    ...
    df = df %>% gather("PatientRooms", "Occupancy2", - c("Emergency" ,"Hour", "Excess", "WaitingPatientRooms", "Occupancy"))
    ...
    #dfSankeyWait = dfSankey
    
    #%>% filter(Column2 != 6)  %>% 
    # filter(Column2 != 7) %>% filter(Column2 != 8)  %>% filter(Column2 != 9)  %>% filter(Column1 != 10)
    
    #dfSankeyWait$Column2 = 13
    
    
    #dfSankey = rbind(dfSankeyWait, dfSankey)
    
    #dfSankeyWait = rbind(dfSankey2, dfSankey1)
    
    #dfSankeyWait = rbind(dfSankeyWait, dfSankey3)
    
    
    
    
    #dfSankeyWait$Column1 = 13
